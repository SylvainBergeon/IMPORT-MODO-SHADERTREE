...
<xsl:template name="advancedMaterial">
	...
	<xsl:variable name="materialName">
		...
		def Material "<xsl:value-of select="$materialName"/>" (
			prepend inherits = &lt;/__class_mtl__/materials/modo/<xsl:value-of select="$materialName"/>&gt;
		)
		{
			token outputs:mtlx:displacement.connect = &lt;/materials/modo/<xsl:value-of select="$materialName"/>/mtlxdisplacement.outputs:out&gt;
			token outputs:mtlx:surface.connect = &lt;/materials/modo/<xsl:value-of select="$materialName"/>/mtlxstandard_surface.outputs:out&gt;
			token outputs:surface.connect = &lt;/materials/modo/<xsl:value-of select="$materialName"/>/mtlxstandard_preview.outputs:surface&gt;
